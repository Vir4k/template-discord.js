...
export default class CommandHandler {
   ...
   */
  static async handleCommand(client: BotClient, message: Message) {
    ...
    if (cmd.info.ArgsMin) {
      if (args.length < cmd.info.ArgsMin) {
        return message.reply({
          embeds: [
            {
              color: "RED",
              title: `Require Arguments`,
              description: `**Name**: ${cmd.info.name}\n**Usage**: ${client.config.prefix}${cmd.info.usage}`,
              fields: [
                {
                  name: "Example Usages",
                  value: cmd.info.examples ? cmd.info.examples.map((x) => `\`${client.config.prefix}${x}\``).join("\n") : "No examples",
                },
              ],
              timestamp: Date.now(),
            },
